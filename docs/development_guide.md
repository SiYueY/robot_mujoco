...
- **运行时读写**：`read()` 通过 `Simulation::read_state(std::shared_ptr<const RobotState>&)` 获取状态快照并更新本地 `JointData` / `ImuData` 等；`write()` 将 controller 命令通过 `Simulation::write_command()` 写入 `CommandBuffer`。
...
- 状态更新走 snapshot 模式：`update_runtime_state()` 调用 `simulation_->read_state(snapshot)` 获取共享指针，然后从 snapshot 中查找各组件状态。这避免了直接访问 `mjData` 的并发问题。
...
- **快照**：`read_state(std::shared_ptr<const RobotState>&)` / `set_snapshot_observer(SnapshotObserver)`
...
│  read():                                                              │
│    -> Simulation::read_state(snapshot)  (从 StateBuffer 读取共享指针)│

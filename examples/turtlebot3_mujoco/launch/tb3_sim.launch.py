...
def load_robot_description(render_mode: str, initial_keyframe: str) -> str:
    package_share_dir = Path(get_package_share_directory("turtlebot3_mujoco"))
    robot_description_template = package_share_dir / "config" / "turtlebot3_waffle_pi.urdf.template"
    mujoco_model_path = package_share_dir / "model" / "scene_turtlebot3_waffle_pi.xml"

    robot_description = robot_description_template.read_text(encoding="utf-8")
...
def launch_setup(context, *args, **kwargs):
    ...
    package_share_dir = Path(get_package_share_directory("turtlebot3_mujoco"))
    controllers_config_path = package_share_dir / "config" / "controllers.yaml"
    rviz_config_path = package_share_dir / "config" / "turtlebot3_mujoco.rviz"
    ...
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                ...
                description="MuJoCo render mode for the TurtleBot3 example.",
            ),
            DeclareLaunchArgument(
                "initial_keyframe",
                default_value="home",
            ...
            DeclareLaunchArgument(
                "use_rviz",
                default_value="true",
                description="Whether to start RViz with the TurtleBot3 example configuration.",
            ),

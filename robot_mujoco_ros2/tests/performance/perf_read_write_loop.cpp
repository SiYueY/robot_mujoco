...
struct TempModelFile {
  explicit TempModelFile(std::string xml_contents) {
    path = std::filesystem::temp_directory_path() / "robot_mujoco_ros2_perf_loop.xml";
...
<mujoco model="hardware_perf_loop">
  ...
  robot_mujoco_ros2::MuJoCoHardwareInterface hardware;
...
int main(int argc, char** argv) { return robot_mujoco_ros2::run_main(argc, argv); }

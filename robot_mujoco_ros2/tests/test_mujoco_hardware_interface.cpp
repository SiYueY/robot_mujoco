 ...
 protected:
  void SetUp() override {
    ros_home_path_ = std::filesystem::temp_directory_path() /
                     ("robot_mujoco_ros2_interface_test_" + std::to_string(::getpid()));
    ...
    if (!rclcpp::ok()) {
      int argc = 1;
      const char* argv[] = {"test_robot_mujoco_ros2_interface", nullptr};
  ...
  std::string write_model(const std::string& xml_contents) {
    ...
    model_path_ = temp_dir / "robot_mujoco_ros2_interface_test.xml";
...
<robot name="test_robot">
  ...
  std::shared_ptr<robot_mujoco_msgs::srv::ResetWorld::Response> call_load_keyframe_service(
    ...
    auto request = std::make_shared<robot_mujoco_msgs::srv::ResetWorld::Request>();
...
<mujoco model="hardware_interface_sensor_test">
  ...
  create_subscriber_node("robot_mujoco_ros2_sensor_listener");
  ...
  auto image_sub = subscriber_node_->create_subscription<sensor_msgs::msg::Image>(
      "/test/camera/image_raw", qos_profiles::sensor_data(),
  ...
  auto lidar_sub = subscriber_node_->create_subscription<sensor_msgs::msg::LaserScan>(
      "/test/lidar/scan", qos_profiles::sensor_data(),
...
<mujoco model="hardware_interface_reset_test">
  ...
  create_subscriber_node("robot_mujoco_ros2_reset_client");
...
<mujoco model="hardware_interface_control_services_test">
  ...
  create_subscriber_node("robot_mujoco_ros2_control_services_client");
...
<mujoco model="hardware_interface_load_keyframe_test">
  ...
  create_subscriber_node("robot_mujoco_ros2_load_keyframe_client");

...
void set_error(ConfigError *error, const XMLElement *element,
               std::string attribute, std::string message) {
  if (error == nullptr || !error->message.empty())
    return;
  error->line = element == nullptr ? 0 : element->GetLineNum();
  error->element =
      element == nullptr || element->Name() == nullptr ? "" : element->Name();
...
bool validate_camera(const CameraConfig &camera, ConfigError *error) {
  ...
  if (height > std::numeric_limits<std::size_t>::max() / width ||
      ...
      width * height * bytes_per_pixel >
          SimulationConfigLimits::kMaximumCameraOutputBytes) {
    set_error(error, nullptr, "width",
              "camera output exceeds the 256 MiB limit");
    return false;
  }
  return true;
}
...
bool validate_lidar(const LidarInfo &lidar, ConfigError *error) {
  ...
  if (lidar.angle_increment <= 0.0) {
    set_error(error, nullptr, "angle_increment",
              "lidar angle_increment must be positive");
    return false;
  }
  if (!(lidar.angle_min < lidar.angle_max)) {
    set_error(error, nullptr, "angle_min",
              "lidar angle_min must be less than angle_max");
    return false;
  }
  if (lidar.range_min < 0.0) {
    set_error(error, nullptr, "range_min",
              "lidar range_min must be non-negative");
    return false;
  }
  if (!(lidar.range_min < lidar.range_max)) {
    set_error(error, nullptr, "range_min",
              "lidar range_min must be less than range_max");
    return false;
  }
  return true;
}
...
bool validate_mobile_base(const MobileBaseInfo &base, ConfigError *error) {
  ...
  if (!std::isfinite(mecanum.wheel_radius) || mecanum.wheel_radius <= 0.0) {
    set_error(error, nullptr, "wheel_radius",
              "wheel_radius must be finite and positive");
    return false;
  }
  if (!std::isfinite(mecanum.wheel_base) || mecanum.wheel_base <= 0.0) {
    set_error(error, nullptr, "wheel_base",
              "wheel_base must be finite and positive");
    return false;
  }
  if (!std::isfinite(mecanum.track_width) || mecanum.track_width <= 0.0) {
    set_error(error, nullptr, "track_width",
              "track_width must be finite and positive");
    return false;
  }
  ...
  for (const WheelInfo &wheel : base.mecanum_wheels) {
    if (wheel.wheel_name.empty() || wheel.actuator_name.empty()) {
      set_error(error, nullptr, "wheel",
                "mobile-base wheel names are required");
      return false;
    }
    if (!std::isfinite(wheel.damping) || wheel.damping < 0.0) {
      set_error(error, nullptr, "damping",
                "mobile-base wheel damping must be finite and non-negative");
      return false;
    }
    if (!wheel_names.insert(wheel.wheel_name).second) {
      set_error(error, nullptr, "name",
                "mobile-base wheel names must be unique");
      return false;
    }
    if (!actuator_names.insert(wheel.actuator_name).second) {
      set_error(error, nullptr, "actuator",
                "mobile-base actuator names must be unique");
      return false;
    }
  }
  return true;
...
bool validate_limit(const Limit &limit, const char *name, ConfigError *error) {
...
template <typename Values> bool values_are_finite(const Values &values) {
...
bool validate_joint(const JointInfo &joint, ConfigError *error) {
  ...
  if (!std::isfinite(joint.position_stiffness) ||
      joint.position_stiffness < 0.0 ||
...
bool validate_imu(const ImuInfo &imu, ConfigError *error) {
  if (imu.name.empty() || imu.frame_id.empty() ||
      imu.framequat_sensor_name.empty() || imu.gyro_sensor_name.empty() ||
      imu.accelerometer_sensor_name.empty()) {
...
bool validate_component_identity(ComponentId id, const std::string &name,
                                 double period,
  ...
  if (id == kInvalidComponentId || id > config.max_component_id ||
    ...
    set_error(error, nullptr, "id",
              std::string(kind) + " ID is outside the configured range");
    return false;
  }
  if (name.empty()) {
...
bool number_array(const XMLElement &element, const char *attribute,
  ...
  for (std::size_t index = 0; index < Size; ++index) {
    ...
    if (!std::getline(values, value, ','))
      return false;
    if (!parse_finite_double(value, out[index]))
      return false;
  }
  std::string extra;
  return !std::getline(values, extra, ',');
}
bool parse_component_id_value(std::string_view raw, ComponentId maximum,
                              ComponentId &out) {
  ...
  return out != kInvalidComponentId &&
         out <= SimulationConfigLimits::kMaximumComponentId && out <= maximum;
}
...
bool parse_joint(const XMLElement &element, ComponentId maximum,
  ...
  if ((stiffness != nullptr &&
       !parse_finite_element_text(*stiffness, info.position_stiffness)) ||
  ...
  for (const XMLElement *e = robot->FirstChildElement("camera"); e != nullptr;
    ...
    if (e->Attribute("enable_depth") != nullptr &&
        e->QueryBoolAttribute("enable_depth", &v.enable_depth) !=
            tinyxml2::XML_SUCCESS) {
      set_error(error, e, "enable_depth",
                "camera output enable attribute must be a boolean");
      return false;
    }
    out.emplace_back(std::move(v));
...
bool parse_simulation(const XMLElement *simulation, SchedulerConfig &config,
                      bool &viewer_enabled, ConfigError *error) {
  ...
  if (physics == nullptr || physics->NextSiblingElement("physics") != nullptr) {
    set_error(error, simulation, "physics",
              "exactly one physics element is required");
    return false;
  }
  if (viewer == nullptr || viewer->NextSiblingElement("viewer") != nullptr) {
    set_error(error, simulation, "viewer",
              "exactly one viewer element is required");
    return false;
  }
  if (!number(*physics, "period", config.physics_period, true)) {
    set_error(error, physics, "period",
              "physics period must be a finite number");
    return false;
  }
  ...
  if (viewer->Attribute("enabled") != nullptr &&
      viewer->QueryBoolAttribute("enabled", &viewer_enabled) !=
          tinyxml2::XML_SUCCESS) {
...
bool SimulationConfigValidator::validate(const SimulationConfig &config,
  ...
  if (config.viewer_startup_timeout <= std::chrono::milliseconds::zero()) {
    set_error(error, nullptr, "viewer_startup_timeout",
  ...
  for (std::size_t component_index = 0;
       component_index < config.components.size(); ++component_index) {
    const ComponentConfig &component = config.components[component_index];
    ConfigError component_error;
    const bool valid = std::visit(
        [&config, &component_error, &joint_ids, &imu_ids, &camera_ids,
         &lidar_ids, &mobile_base_ids, &joint_names, &imu_names, &camera_names,
         &lidar_names, &mobile_base_names](const auto &info) {
          using Info = std::decay_t<decltype(info)>;
          if constexpr (std::is_same_v<Info, JointInfo>) {
            return validate_component_identity(
                       info.id, info.joint_name, info.period, joint_ids,
                       joint_names, "joint", config, &component_error) &&
...
bool SimulationConfigParser::load_file(const std::string &path,
  ...
  if (document.LoadFile(path.c_str()) != tinyxml2::XML_SUCCESS) {
    if (error != nullptr) {
      ...
      error->message = document.ErrorStr() == nullptr ? "failed to load XML"
                                                      : document.ErrorStr();
    }
    return false;
  }
  ...
  if (root == nullptr || std::string(root->Name()) != "robot_mujoco" ||
    ...
    set_error(error, root, "",
              "expected a robot_mujoco root with known children");
    return false;
  }
  SimulationConfig parsed;
  if (const char *max = root->Attribute("max_component_id")) {
    if (!parse_component_id_value(max,
                                  SimulationConfigLimits::kMaximumComponentId,
  ...
  if (mujoco == nullptr || mjcf == nullptr || !allowed(*mujoco, {"mjcf"})) {
    set_error(error, mujoco == nullptr ? root : mujoco, "",
              "expected one mjcf element");
    return false;
  }
  ...
  if (simulation == nullptr ||
    ...
    set_error(error, simulation == nullptr ? root : simulation, "simulation",
              "exactly one simulation element is required");
    return false;
  }
  if (!parse_simulation(simulation, parsed.scheduler, parsed.viewer_enabled,
                        error)) {
    return false;
  }
  ...
  if (!parse_components(root->FirstChildElement("robot"),
    ...
    if (error != nullptr && error->message.empty())
      set_error(error, root->FirstChildElement("robot"), "",
  ...
  if (!SimulationConfigValidator::validate(parsed, &validation_error)) {
    if (error != nullptr) {
      *error = std::move(validation_error);
      ...
      } else if (error->attribute == "physics_period" ||
                 error->attribute == "viewer_period") {

...
MobileBaseComponent::MobileBaseComponent(MobileBaseInfo info)
    : SimulationComponent(info.mobile_base_name, info.update_rate), info_(std::move(info)) {}
...
bool MobileBaseComponent::write(const mjContext& context, const MobileBaseCommand& command) {
  ...
  if (command.mobile_base_name != info_.mobile_base_name) {
    LOG_ERROR << "mobile base '" << info_.mobile_base_name << "' received a command for '"
              << command.mobile_base_name << "'.";
    return false;
  }
...
bool MobileBaseComponent::write_mecanum_command(const mjContext& context,
  ...
  switch (command.mode) {
    ...
    default:
      LOG_ERROR << "mobile base '" << info_.mobile_base_name
                << "' received unsupported mecanum control mode " << static_cast<int>(command.mode)
                << ".";
      return false;
  }
}

bool MobileBaseComponent::configure_wheel(const mjContext& context, const WheelInfo& info,
                                          mjWheel& wheel) const {
...
bool MobileBaseComponent::write_twist_command(const mjContext& context,
                                              const MobileBaseCommand& command) {
...
bool MobileBaseComponent::write_wheel_linear_command(const mjContext& context,
                                                     const MobileBaseCommand& command) {
  Vector4d wheel_angular{};
  for (std::size_t wheel_index = 0; wheel_index < MecanumWheelCount; ++wheel_index) {
...
bool MobileBaseComponent::write_wheel_angular_command(const mjContext& context,
                                                      const MobileBaseCommand& command) {
  for (std::size_t wheel_index = 0; wheel_index < MecanumWheelCount; ++wheel_index) {
...
double MobileBaseComponent::clamp_ctrl_limits(const mjContext& context, const mjWheel& wheel,
                                              double value) const {
...
double MobileBaseComponent::clamp_force_limits(const mjContext& context, const mjWheel& wheel,
                                               double value) const {

...
bool MobileBaseComponent::write(const mjContext &context,
...
bool MobileBaseComponent::read_state(
    std::shared_ptr<const MobileBaseState> &state) const {
  if (!is_initialized()) {
    LOG_ERROR << "mobile base '" << info_.mobile_base_name
              << "' is not initialized.";
    return false;
...
bool MobileBaseComponent::read(const mjContext &context,
...
bool MobileBaseComponent::is_initialized() const noexcept {
  return initialized_;
}
...
bool MobileBaseComponent::configure_base_body(const mjContext &context) {
  ...
  base_body_id_ =
      mj_name2id(context.model, mjOBJ_BODY, info_.base_body_name.c_str());
  if (base_body_id_ < 0) {
    LOG_ERROR << "mobile base '" << info_.mobile_base_name << "' body '"
              << info_.base_body_name << "' was not found.";
    return false;
  }
  return true;
}
...
bool MobileBaseComponent::init_mecanum(const mjContext &context) {
  ...
  const double rotation_coefficient =
      (info.wheel_base + info.track_width) * 0.5;
  ...
  for (std::size_t wheel_index = 0; wheel_index < MecanumWheelCount;
       ++wheel_index) {
    if (!configure_wheel(context, info_.mecanum_wheels[wheel_index],
                         mecanum_wheels_[wheel_index])) {
      return false;
    }
  }
...
bool MobileBaseComponent::update_mecanum_state(const mjContext &context) {
  ...
  for (std::size_t wheel_index = 0; wheel_index < MecanumWheelCount;
    ...
    wheel_angular[wheel_index] =
        context.data->qvel[mecanum_wheels_[wheel_index].dof_address];
...
bool MobileBaseComponent::configure_wheel(const mjContext &context,
                                          const WheelInfo &info,
                                          mjWheel &wheel) const {
  if (info.wheel_name.empty()) {
    LOG_ERROR << "mobile base '" << info_.mobile_base_name
              << "' wheel name must not be empty.";
    return false;
  }
  if (info.actuator_name.empty()) {
    LOG_ERROR << "mobile base '" << info_.mobile_base_name << "' wheel '"
              << info.wheel_name << "' actuator name must not be empty.";
    return false;
  }
  if (!std::isfinite(info.damping) || info.damping <= 0.0) {
    LOG_ERROR << "mobile base '" << info_.mobile_base_name << "' wheel '"
              << info.wheel_name << "' damping must be finite and positive.";
    return false;
  }

  mjWheel binding{};
  binding.wheel_id =
      mj_name2id(context.model, mjOBJ_JOINT, info.wheel_name.c_str());
  if (binding.wheel_id < 0) {
    LOG_ERROR << "mobile base '" << info_.mobile_base_name << "' wheel '"
              << info.wheel_name << "' was not found.";
    return false;
  }
  if (context.model->jnt_type[binding.wheel_id] != mjJNT_HINGE) {
    LOG_ERROR << "mobile base '" << info_.mobile_base_name << "' wheel '"
              << info.wheel_name << "' must be a MuJoCo hinge joint.";
    return false;
  }
  binding.dof_address = context.model->jnt_dofadr[binding.wheel_id];
  if (binding.dof_address < 0) {
    LOG_ERROR << "mobile base '" << info_.mobile_base_name << "' wheel '"
              << info.wheel_name << "' has an invalid DOF address.";
    return false;
  }

  binding.actuator_id =
      mj_name2id(context.model, mjOBJ_ACTUATOR, info.actuator_name.c_str());
  ...
  if (context.model->actuator_trnid[2 * binding.actuator_id] !=
      binding.wheel_id) {
    LOG_ERROR << "mobile base '" << info_.mobile_base_name << "' actuator '"
  ...
  const mjtNum *gain =
      context.model->actuator_gainprm + binding.actuator_id * mjNGAIN;
...
bool MobileBaseComponent::write_twist_command(
    const mjContext &context, const MobileBaseCommand &command) {
  if (!std::isfinite(command.base_linear[0]) ||
  ...
  Vector4d wheel_angular{};
  mecanum_kinematics_->inverse(command.base_linear, command.base_angular,
                               wheel_angular);
...
bool MobileBaseComponent::write_wheel_commands(const mjContext &context,
                                               const Vector4d &wheel_angular) {
  for (std::size_t wheel_index = 0; wheel_index < MecanumWheelCount;
       ++wheel_index) {
    const mjWheel &wheel = mecanum_wheels_[wheel_index];
    double effort =
        info_.mecanum_wheels[wheel_index].damping *
        (wheel_angular[wheel_index] - context.data->qvel[wheel.dof_address]);
    effort = clamp_force_limits(context, wheel, effort);
    context.data->ctrl[wheel.actuator_id] =
        clamp_ctrl_limits(context, wheel, effort);
  }
  return true;
}

double MobileBaseComponent::clamp_ctrl_limits(const mjContext &context,
                                              const mjWheel &wheel,
                                              double value) const {
  ...
  const mjtNum *range =
      context.model->actuator_ctrlrange + 2 * wheel.actuator_id;
  return std::clamp(value, static_cast<double>(range[0]),
                    static_cast<double>(range[1]));
}

double MobileBaseComponent::clamp_force_limits(const mjContext &context,
                                               const mjWheel &wheel,
                                               double value) const {
  ...
  const mjtNum *range =
      context.model->actuator_forcerange + 2 * wheel.actuator_id;
  return std::clamp(value, static_cast<double>(range[0]),
                    static_cast<double>(range[1]));
}
...
bool MobileBaseComponent::update_ground_truth_pose(const mjData &data) {
  if (base_body_id_ < 0) {
    LOG_ERROR << "mobile base '" << info_.mobile_base_name
              << "' body binding is missing.";
    return false;
  }
  const mjtNum *xpos = data.xpos + 3 * base_body_id_;
  const mjtNum *xmat = data.xmat + 9 * base_body_id_;
  working_state_.pose = {static_cast<double>(xpos[0]),
                         static_cast<double>(xpos[1]),
                         wrap_angle(std::atan2(static_cast<double>(xmat[3]),

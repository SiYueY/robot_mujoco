...
public:
  ...
  virtual bool initialize(const SimulationConfig &config,
                          const mjModel *model) = 0;
  virtual CameraRenderSubmitResult
  submit(const CameraRenderBatchRequest &request,
         CameraRenderTicket &ticket) = 0;
  virtual CameraRenderWaitStatus wait(const CameraRenderTicket &ticket,
                                      std::chrono::milliseconds timeout) = 0;
  virtual CameraRenderWaitStatus
  query(const CameraRenderTicket &ticket) const = 0;
  virtual bool read_batch_result(const CameraRenderTicket &ticket,

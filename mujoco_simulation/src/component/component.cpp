...
bool SimulationComponent::configure(const mjContext &context) {
  ...
  if (period_ > 0.0 && period_ < physics_period) {
    LOG_ERROR << "component '" << name_ << "' period " << period_
              << " must not be shorter than the physics period "
              << physics_period << ".";
    return false;
  }
  ...
  if (period_ > 0.0) {
    ...
    if (rounded_multiple < 1.0 ||
        std::abs(multiple - rounded_multiple) > tolerance) {
      LOG_ERROR << "component '" << name_ << "' period " << period_
                << " must be an integer multiple of the physics period "
                << physics_period << ".";
      return false;
    }
  }
  ...
  if (period_ == 0.0) {
    LOG_DEBUG << "component '" << name_
              << "' is configured to update every physics step.";
  } else {
    LOG_DEBUG << "component '" << name_ << "' is configured with period "
              << period_ << " seconds.";
  }

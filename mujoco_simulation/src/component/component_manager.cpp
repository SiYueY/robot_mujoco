...
bool ComponentManager::init(const mjContext &context,
  ...
  for (const ComponentConfig &component : components) {
    if (const auto *joint = std::get_if<JointInfo>(&component)) {
      if (!register_component(context,
                              std::make_unique<JointComponent>(*joint))) {
        clear();
        return false;
      }
    }
  ...
  for (const ComponentConfig &component : components) {
    if (const auto *lidar = std::get_if<LidarInfo>(&component)) {
      if (!register_component(context,
                              std::make_unique<LidarComponent>(*lidar))) {
        clear();
        return false;
      }
    }
  ...
  for (const ComponentConfig &component : components) {
    if (const auto *mobile_base = std::get_if<MobileBaseInfo>(&component)) {
      auto mobile_base_component =
          std::make_unique<MobileBaseComponent>(*mobile_base);
  ...
  for (const ComponentConfig &component : components) {
    if (const auto *camera = std::get_if<CameraConfig>(&component)) {
      if (!register_component(context,
                              std::make_unique<CameraComponent>(*camera),
                              camera_renderer)) {
        clear();
        return false;
      }
...
bool ComponentManager::register_component(
    const mjContext &context, MobileBaseComponent::UniquePtr mobile_base) {
...
bool ComponentManager::reset(const mjContext &context) {
  ...
  for (const auto &[name, mobile_base_component] :
       component_registry.mobile_bases()) {
    UNUSED(name);
...
bool ComponentManager::update(const mjContext &context) {
  ...
  if (!due_joints.empty()) {
    auto states = std::make_shared<JointStateMap>(
        joints_ == nullptr ? JointStateMap{} : *joints_);
  ...
  if (!due_imus.empty()) {
    auto states = std::make_shared<ImuStateMap>(imus_ == nullptr ? ImuStateMap{}
                                                                 : *imus_);
  ...
  if (!due_lidars.empty()) {
    auto states = std::make_shared<LidarStateMap>(
        lidars_ == nullptr ? LidarStateMap{} : *lidars_);
  ...
  if (!due_cameras.empty()) {
    auto states = std::make_shared<CameraStateMap>(
        cameras_ == nullptr ? CameraStateMap{} : *cameras_);
...
bool ComponentManager::write_command(const mjContext &context,
  ...
  for (const auto &[mobile_base_name, command] :
       snapshot.mobile_base_commands) {
    MobileBaseComponent *mobile_base_component =
        component_registry.mobile_base(mobile_base_name);
...
bool ComponentManager::read_state(const mjContext &context,

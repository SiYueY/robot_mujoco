...
bool CameraRenderer::render(const mjContext& context, const CameraConfig& spec,
  ...
  mjv_updateScene(context.model, render_data_, &option_, nullptr, &camera, mjCAT_ALL, &scene_);
  ...
  if (spec.enable_depth) {
    ...
    if (!transform_depth(context, depth_buffer, state->depth.width, state->depth.height,
                         state->depth.data)) {
      return false;
    }
  }
...
bool CameraRenderer::resize_offscreen_buffer(int width, int height) {
  ...
  if (!pixel_count_for(width, height, pixel_count)) {
    LOG_ERROR << "offscreen image dimensions must be positive and not overflow pixel capacity.";
    return false;
  }
  UNUSED(pixel_count);
...
void CameraRenderer::transform_rgb(const std::vector<std::uint8_t>& source, std::uint32_t width,
...
bool CameraRenderer::transform_depth(const mjContext& context, const std::vector<float>& source,
                                     std::uint32_t width, std::uint32_t height,
                                     std::vector<float>& dest) const {
  const float near = static_cast<float>(context.model->vis.map.znear * context.model->stat.extent);
  const float far = static_cast<float>(context.model->vis.map.zfar * context.model->stat.extent);
  ...
  for (std::uint32_t row = 0; row < height; ++row) {
    for (std::uint32_t column = 0; column < width; ++column) {
      ...
      if (!std::isfinite(denominator) || denominator <= 0.0F) {
        LOG_ERROR << "camera depth buffer contains an invalid normalized depth value.";
        return false;
      }
...
void CameraRenderer::deactivate_context() {
  if (gl_context_.backend == OffscreenGlBackend::Glfw && gl_context_.window != nullptr) {
    ...
    UNUSED(eglMakeCurrent(static_cast<::EGLDisplay>(gl_context_.egl_display), EGL_NO_SURFACE,

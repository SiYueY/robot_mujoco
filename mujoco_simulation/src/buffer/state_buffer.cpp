...
bool read_by_id(const StateSnapshots<State>& states, ComponentId id, State& out) {
...
bool StateBuffer::read_mobile_base_state(MobileBaseId id, MobileBaseState& out) const {
    const auto snapshot = read();
...
void StateBuffer::clear() {
    std::atomic_store_explicit(
        &current_, std::shared_ptr<const RobotState>{}, std::memory_order_release);
}

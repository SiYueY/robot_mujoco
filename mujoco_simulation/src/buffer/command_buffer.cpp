...
bool CommandBuffer::write(const RobotCommand &command) {
  ...
  if (!initialized_)
    return false;
  if (command.joints.empty() && command.mobile_bases.empty())
    return true;
  ...
  if (joint_channel == nullptr || mobile_base_channel == nullptr ||
      (!command.joints.empty() &&
       !joint_channel->channel.validate(command.joints)) ||
  ...
  if (!command.mobile_bases.empty()) {
    mobile_bases =
        mobile_base_channel->channel.updated_snapshot(command.mobile_bases);
    updated.channels_.at(std::type_index(typeid(MobileBaseCommand))) =
        mobile_bases;
  }
  ++updated.sequence;

  if (joints != nullptr)
    joint_channel->channel.replace_snapshot(std::move(joints));

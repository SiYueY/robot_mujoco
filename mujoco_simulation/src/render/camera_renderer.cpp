...
CameraFrame frame_from_render_state(const CameraRenderState &state) {
  ...
  if (!state.depth.data.empty()) {
    std::memcpy(frame.depth_image.data.data(), state.depth.data.data(),
                frame.depth_image.data.size());
  }
...
bool CameraRenderer::initialize(const mjModel *model) {
  ...
  const auto startup_deadline =
      std::chrono::steady_clock::now() + kWorkerTimeout;
...
std::optional<CameraRenderTicket>
CameraRenderer::submit(const mjModel *model, const mjData *data,
                       std::vector<CameraRenderTask> tasks) {
  CameraRenderBatchRequest request;
  request.model = model;
  request.data = data;
...
std::optional<CameraRenderTicket>
CameraRenderer::submit(const CameraRenderBatchRequest &request,
                       bool *replaced_pending_batch) {
  if (replaced_pending_batch != nullptr)
    *replaced_pending_batch = false;
  ...
  CameraRenderTicket ticket;
  {
    ...
    if (pending_ready_) {
      ...
      for (const CameraRenderTask &pending : pending_tasks_) {
        superseded.cameras.push_back({pending.config.id,
...
CameraRenderWaitStatus CameraRenderer::wait_result(CameraRenderTicket requested,
                                                   CameraBatchResult *result) {
  return wait_result(
      requested,
      std::chrono::duration_cast<std::chrono::milliseconds>(kWorkerTimeout),
      result);
}

CameraRenderWaitStatus
CameraRenderer::wait_result(CameraRenderTicket requested,
  ...
  const bool completed = completion_condition_.wait_for(
      lock, timeout, [this, requested_generation, requested_key] {
  ...
  if (!batch.all_succeeded) {
    ...
    const bool superseded =
        !batch.cameras.empty() &&
        std::all_of(batch.cameras.begin(), batch.cameras.end(),
                    [](const CameraRenderTaskResult &camera) {
                      return camera.status == CameraTaskStatus::Superseded;
                    });
    if (superseded)
      return CameraRenderWaitStatus::Superseded;
...
CameraRenderWaitStatus CameraRenderer::query(CameraRenderTicket requested,
                                             CameraBatchResult *result) const {
  std::lock_guard<std::mutex> lock(job_mutex_);
  if (requested.is_noop())
    return CameraRenderWaitStatus::Completed;
  if (requested.generation != ticket_epoch_.load() ||
      requested.sequence > submitted_ticket_) {
    return CameraRenderWaitStatus::InvalidTicket;
  }
  if (requested.generation == expired_through_ticket_.generation &&
      requested.sequence <= expired_through_ticket_.sequence) {
  ...
  const auto completed =
      completed_results_.find({requested.generation, requested.sequence});
  if (completed == completed_results_.end())
    return stopping_ || worker_initialization_failed_
               ? CameraRenderWaitStatus::Stopped
               : CameraRenderWaitStatus::Timeout;
  if (result != nullptr)
    *result = completed->second;
  if (completed->second.all_succeeded)
    return CameraRenderWaitStatus::Completed;
  const bool superseded =
      !completed->second.cameras.empty() &&
      std::all_of(completed->second.cameras.begin(),
                  completed->second.cameras.end(),
                  [](const CameraRenderTaskResult &camera) {
                    return camera.status == CameraTaskStatus::Superseded;
                  });
  if (superseded)
      ...
      if (succeeded) {
        ...
        for (const CameraRenderTask &task : tasks) {
          ...
          if (!render_task(task, state, &error)) {
            succeeded = false;
            batch.cameras.push_back({task.config.id,
          ...
          batch.cameras.push_back(
              {task.config.id, CameraTaskStatus::Completed,
               frame_from_render_state(*state), batch.ticket.generation,
               batch.ticket.sequence, batch.simulation_step,
...

      {
        ...
        if (batch.all_succeeded) {
          ...
          const bool any_success =
              std::any_of(batch.cameras.begin(), batch.cameras.end(),
                          [](const CameraRenderTaskResult &camera) {
                            return camera.status == CameraTaskStatus::Completed;
                          });

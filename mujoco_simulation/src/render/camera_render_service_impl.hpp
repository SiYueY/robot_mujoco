...
public:
    ...
    bool initialize(const SimulationConfig& config, const mjModel* model) override;
    CameraRenderSubmitResult submit(
        const CameraRenderBatchRequest& request, CameraRenderTicket& ticket) override;
    CameraRenderWaitStatus wait(
        const CameraRenderTicket& ticket, std::chrono::milliseconds timeout) override;

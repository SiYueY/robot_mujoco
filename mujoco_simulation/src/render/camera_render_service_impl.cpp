...
bool CameraRenderServiceImpl::initialize(const SimulationConfig& config, const mjModel* model) {
    (void)config;
    return renderer_.initialize(model);
}

CameraRenderSubmitResult CameraRenderServiceImpl::submit(
    const CameraRenderBatchRequest& request, CameraRenderTicket& ticket) {
...
CameraRenderWaitStatus CameraRenderServiceImpl::wait(
    const CameraRenderTicket& ticket, std::chrono::milliseconds timeout) {
    return renderer_.wait_result(ticket, timeout);
}

CameraRenderWaitStatus CameraRenderServiceImpl::query(const CameraRenderTicket& ticket) const {

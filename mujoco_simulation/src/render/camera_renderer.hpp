...
public:
  ...
  std::optional<CameraRenderTicket> submit(const mjModel *model,
                                           const mjData *data,
                                           std::vector<CameraRenderTask> tasks);
  ...
  submit(const CameraRenderBatchRequest &request,
         bool *replaced_pending_batch = nullptr);
  /// Wait for the exact batch identified by ticket and report why it failed.
  CameraRenderWaitStatus wait_result(CameraRenderTicket ticket,
                                     CameraBatchResult *result = nullptr);
  CameraRenderWaitStatus wait_result(CameraRenderTicket ticket,
                                     std::chrono::milliseconds timeout,
                                     CameraBatchResult *result = nullptr);
  CameraRenderWaitStatus query(CameraRenderTicket ticket,
                               CameraBatchResult *result = nullptr) const;
  ...
  std::uint64_t snapshot_copy_count() const noexcept {
    return snapshot_copy_count_.load();
  }

...
bool Simulation::write_command(std::string name, const JointCommand &command) {
  ...
  return command_buffer->write_joint_command(resolved_command.joint_name,
                                             resolved_command);
}
...
bool Simulation::initialize_components() {
  {
    ...
    if (camera_renderer_ == nullptr ||
        !component_manager_.init(context, config_.components,
                                 *camera_renderer_)) {
      return false;
    }
...
bool Simulation::scheduler_write_commands() {
  std::lock_guard<std::mutex> lock(runtime_mutex_);
  if (command_buffer_ == nullptr || runtime_ == nullptr ||
      !runtime_->is_initialized()) {
    return false;
  }
...
bool Simulation::update_components_for_step_locked(std::uint64_t step,
...
bool Simulation::write_state_snapshot_locked(
    std::shared_ptr<const RobotState> &published_snapshot) {
  if (state_buffer_ == nullptr || runtime_ == nullptr ||
      !runtime_->is_initialized()) {
    return false;
  }

  const double simulation_time = runtime_->time();
  return build_state_snapshot_locked(step_, simulation_time,
                                     published_snapshot);
}

...
bool SimulationRuntime::load_model(const std::string &file_path,
                                   mjModel *&model) {
  model = nullptr;
  if (file_path.empty()) {
  ...
  mj_model =
      mj_loadXML(path.c_str(), nullptr, error_message, error_message_length);
  if (mj_model == nullptr) {
    LOG_ERROR << "failed to load xml model file '" << path
              << "': " << error_message;
    return false;
  }
...
bool SimulationRuntime::init(const ModelConfig &config) {
  ...
  if (!config.initial_keyframe.empty()) {
    ...
    if (keyframe_id < 0) {
      LOG_ERROR << "keyframe '" << config.initial_keyframe
                << "' was not found.";
      context_.clear();
      return false;
    }
...
const mjContext &SimulationRuntime::context() const noexcept {
  return context_;
}

...
struct SimulationViewer::ViewerSnapshot::Lease {
    Lease(std::shared_ptr<SnapshotPool> pool, mjData* data) : pool(std::move(pool)), data(data) {}
    ~Lease() {
        if (pool != nullptr) pool->release(data);
    }
...
SimulationViewer::ViewerSnapshot::~ViewerSnapshot() = default;
SimulationViewer::ViewerSnapshot::ViewerSnapshot(ViewerSnapshot&&) noexcept = default;
SimulationViewer::ViewerSnapshot& SimulationViewer::ViewerSnapshot::operator=(
    ViewerSnapshot&&) noexcept = default;
SimulationViewer::ViewerSnapshot::operator bool() const noexcept { return lease_ != nullptr; }

std::unique_ptr<SimulationViewer::ViewerSnapshot::Lease> SimulationViewer::SnapshotPool::acquire() {
    std::lock_guard<std::mutex> lock(mutex);
    if (!accepting || available.empty()) return nullptr;
...
void SimulationViewer::SnapshotPool::shutdown() {
    std::lock_guard<std::mutex> lock(mutex);
    accepting = false;
    for (mjData* data : available) mj_deleteData(data);
    available.clear();
}
...
void SimulationViewer::render_task(
    ...
    try {
        ...
        auto simulate = std::make_unique<mujoco::Simulate>(
            std::make_unique<mujoco::GlfwAdapter>(), &camera_, &visual_options_, &perturb_, true);
...

        {
            ...
            simulate->dnew_ = data;
            std::strncpy(
                simulate->filename, displayed_filename.c_str(), sizeof(simulate->filename) - 1);
...
bool SimulationViewer::start(const std::string& displayed_filename) {
    ...
    {
        std::lock_guard<std::mutex> lock(mutex_);
        if (state_ == ViewerState::Ready) return true;
    }

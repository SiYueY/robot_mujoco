...
GlfwAdapter::GlfwAdapter() {
    ...
    // create window
    window_ = Glfw().glfwCreateWindow(
        (2 * vidmode_.width) / 3, (2 * vidmode_.height) / 3, "MuJoCo", nullptr, nullptr);
    if (!window_) {
...
void GlfwAdapter::ToggleFullscreen() {
    ...
    if (Glfw().glfwGetWindowMonitor(window_)) {
        ...
        Glfw().glfwSetWindowMonitor(
            window_, nullptr, window_pos_.first, window_pos_.second, window_size_.first,
    ...
    // currently windowed: switch to full screen
        ...
        // switch
        Glfw().glfwSetWindowMonitor(
            window_, Glfw().glfwGetPrimaryMonitor(), 0, 0, vidmode_.width, vidmode_.height,

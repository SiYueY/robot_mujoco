...
void UpdateProfiler(mj::Simulate *sim, const mjModel *m, const mjData *d) {
  ...
  float sdata[6] = {
      static_cast<float>(nv),      static_cast<float>(nbody),
...
void UpdateSensorImage(mj::Simulate *sim, const mjModel *m, const mjData *d) {
  if (sim->selected_image_sensor < 0 || !sim->sensor_image)
    return;
...
void MakePhysicsSection(mj::Simulate *sim) {
  ...
  mjuiDef defPhysics[] = {
      ...
      {mjITEM_END}};
  mjuiDef defEnableFlags[] = {{mjITEM_SEPARATOR, "Enable Flags", mjPRESERVE},
                              {mjITEM_END}};
  mjuiDef defOverride[] = {
...
void MakeVisualizationSection(mj::Simulate *sim, const mjModel *m) {
  mjStatistic *stat =
      sim->is_passive_ ? &sim->m_passive_->stat : &sim->m_->stat;
...
void MakeEqualitySection(mj::Simulate *sim) {
  mjuiDef defEquality[] = {
      {mjITEM_SECTION, "Equality", mjPRESERVE, nullptr, "AE"}, {mjITEM_END}};
  mjuiDef defCheckBox[] = {{mjITEM_CHECKBYTE, "", 2, nullptr, ""},
                           {mjITEM_END}};
  ...
  if ((state->dragrect == sim->ui0.rectid) ||
    ...
    // simulation section
      ...
      case 4: // Copy key
        ...
        sim->pending_.copy_key_full_precision =
...
void Simulate::Sync(bool state_only) {
  ...
  if (update_sensor) {
    ...
    int new_selected =
        this->image_sensor_ui_selection - 1; // 0=None(-1), 1+=sensor
...
void Simulate::Render() {
  ...
  if (pending_.ui_update_equality) {
    if (this->ui1_enable && this->ui1.sect[SECT_EQUALITY].state) {
      mjui_update(SECT_EQUALITY, -1, &this->ui1, &this->uistate,
                  &this->platform_ui->mjr_context());
    }
  ...
  while (!this->platform_ui->ShouldCloseWindow() && !this->exitrequest.load()) {
    {
      ...
      if (!is_passive_) {
        ...
        mjv_updateScene(m_passive_, d_passive_, &this->opt, &this->pert,

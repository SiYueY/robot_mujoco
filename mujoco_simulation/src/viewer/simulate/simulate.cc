...
void UpdateProfiler(mj::Simulate* sim, const mjModel* m, const mjData* d) {
    ...
    float sdata[6] = {static_cast<float>(nv),      static_cast<float>(nbody),
...
void ShowProfiler(mj::Simulate* sim, mjrRect rect) {
    mjrRect viewport = {
        rect.left + rect.width - rect.width / 4, rect.bottom, rect.width / 4, rect.height / 4};
...
void ShowSensor(mj::Simulate* sim, mjrRect rect) {
    ...
    if (sim->image_sensor_count > 0 && sim->selected_image_sensor >= 0 && sim->sensor_image) {
        // render sensor image - viewport dimensions MUST match image buffer
        // dimensions
        ...
        if (img_w > 0 && img_h > 0) {
            ...
            mjr_drawPixels(
                sim->sensor_image.get(), nullptr, img_viewport, &sim->platform_ui->mjr_context());
        }
    } else {
...
void InitializeSensorImage(mj::Simulate* sim, const mjModel* m) {
    if (sim->selected_image_sensor < 0 || sim->selected_image_sensor >= sim->image_sensor_count) {
        sim->sensor_image.reset();
        return;
    }
...
void ShowOverlayText(
    mj::Simulate* sim, mjrRect viewport, int font, int gridpos, std::string text1,
    std::string text2) {
    mjr_overlay(
        font, gridpos, viewport, text1.c_str(), text2.c_str(), &sim->platform_ui->mjr_context());
}
...
// prepare info text
void UpdateInfoText(
    mj::Simulate* sim, const mjModel* m, const mjData* d,
...
void MakeRenderingSection(mj::Simulate* sim, const mjModel* m) {
    ...
    if (sim->image_sensor_count > 0) {
        ...
        mjuiDef defImageSensor[] = {
            {mjITEM_SELECT, "Sensor", 2, &sim->image_sensor_ui_selection, ""}, {mjITEM_END}};
    ...
    for (int i = 0; i < sim->jnt_type_.size() && itemcnt < mjMAXUIITEM; i++) {
        if ((sim->jnt_type_[i] == mjJNT_HINGE || sim->jnt_type_[i] == mjJNT_SLIDE)) {
            ...
            if (sim->jnt_range_[i].has_value())
                mju::sprintf_arr(
                    defSlider[0].other, "%.17g %.17g", sim->jnt_range_[i]->first,
...
void MakeControlSection(mj::Simulate* sim) {
    ...
    for (int i = 0; i < sim->actuator_ctrlrange_.size() && itemcnt < mjMAXUIITEM; i++) {
        ...
        if (sim->actuator_ctrlrange_[i].has_value())
            mju::sprintf_arr(
                defSlider[0].other, "%.17g %.17g", sim->actuator_ctrlrange_[i]->first,
...
Simulate::Simulate(
    std::unique_ptr<PlatformUIAdapter> platform_ui, mjvCamera* cam, mjvOption* opt,
...
void Simulate::Sync(bool state_only) {
    ...
    if (pending_.select) {
        ...
        int selbody = mjv_select(
            m_, d_, &this->opt, static_cast<mjtNum>(r.width) / r.height,
            (pending_.select_state.x - r.left) / r.width,
            (pending_.select_state.y - r.bottom) / r.height, &this->scn, selpnt, &selgeom, &selflex,
            &selskin);
...
void Simulate::LoadOnRenderThread() {
    ...
    std::memcpy(
        body_parentid_.data(), this->m_->body_parentid,
        sizeof(this->m_->body_parentid[0]) * this->m_->nbody);

    jnt_type_.resize(this->m_->njnt);
    std::memcpy(
        jnt_type_.data(), this->m_->jnt_type, sizeof(this->m_->jnt_type[0]) * this->m_->njnt);
    ...
    std::memcpy(
        jnt_qposadr_.data(), this->m_->jnt_qposadr,
    ...
    std::memcpy(
        actuator_group_.data(), this->m_->actuator_group,
    ...
    std::memcpy(
        eq_active_.data(), this->d_->eq_active, sizeof(this->d_->eq_active[0]) * this->m_->neq);
    ...
    // update scene in managed mode, in passive mode copy data from user (update
    // in RenderLoop)
    if (!is_passive_) {
        mjv_updateScene(
            this->m_, this->d_, &this->opt, &this->pert, &this->cam, mjCAT_ALL, &this->scn);
    } else {
        mjopt_prev_ = m_->opt;
        opt_prev_ = opt;
        cam_prev_ = cam;
...
void Simulate::Render() {
    ...
    if (!this->is_passive_ && !this->m_) {
        ...
        if (this->loadrequest) {
            mjr_overlay(
                mjFONT_BIG, mjGRID_TOP, smallrect, "LOADING...", nullptr,
            ...
            mju::sprintf_arr(
                ...
                mj_versionString());
            mjr_overlay(
                mjFONT_NORMAL, mjGRID_TOPLEFT, rect, intro_message, 0,
                &this->platform_ui->mjr_context());
        }
        ...
        if (this->load_error[0]) {
            mjr_overlay(
                mjFONT_NORMAL, mjGRID_BOTTOMLEFT, rect, this->load_error, 0,
                &this->platform_ui->mjr_context());
        }
    ...
    if (pending_.ui_update_joint) {
        if (this->ui1_enable && this->ui1.sect[SECT_JOINT].state) {
            mjui_update(
                SECT_JOINT, -1, &this->ui1, &this->uistate, &this->platform_ui->mjr_context());
        }
    ...
    if (pending_.ui_update_ctrl) {
        if (this->ui1_enable && this->ui1.sect[SECT_CONTROL].state) {
            mjui_update(
                SECT_CONTROL, -1, &this->ui1, &this->uistate, &this->platform_ui->mjr_context());
        }
    ...
    if (pending_.ui_update_equality) {
        if (this->ui1_enable && this->ui1.sect[SECT_EQUALITY].state) {
            mjui_update(
                SECT_EQUALITY, -1, &this->ui1, &this->uistate, &this->platform_ui->mjr_context());
        }
    ...
    if (rtlabel[0]) {
        mjr_overlay(
            mjFONT_BIG, mjGRID_TOPLEFT, smallrect, rtlabel, nullptr,
            &this->platform_ui->mjr_context());
    }
    ...
    if (this->help) {
        mjr_overlay(
            mjFONT_NORMAL, mjGRID_TOPLEFT, rect, help_title, help_content,
            &this->platform_ui->mjr_context());
    }
    ...
    if (this->info) {
        mjr_overlay(
            mjFONT_NORMAL, mjGRID_BOTTOMLEFT, rect, this->info_title, this->info_content,
            &this->platform_ui->mjr_context());
    }
    ...
    while (!this->platform_ui->ShouldCloseWindow() && !this->exitrequest.load()) {
        {
            ...
            if (!is_passive_) {
                ...
                mjv_updateScene(
                    m_passive_, d_passive_, &this->opt, &this->pert, &this->cam, mjCAT_ALL,
                    &this->scn);
                ...
                std::memcpy(
                    this->scn.geoms + this->scn.ngeom, user_scn_geoms_.data(),

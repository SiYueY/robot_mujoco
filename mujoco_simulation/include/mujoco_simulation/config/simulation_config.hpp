...
private:
  ...
  static bool parse_required_double(const tinyxml2::XMLElement &element,
                                    double &out);
  static bool
  reject_unknown_children(const tinyxml2::XMLElement &element,
                          const std::unordered_set<std::string> &allowed_names);
  static bool parse_limit_axis(const tinyxml2::XMLElement *element,
                               Limit &limits);
  static bool parse_position_config(const tinyxml2::XMLElement *element,
                                    JointInfo &info);
  static bool parse_velocity_config(const tinyxml2::XMLElement *element,
                                    JointInfo &info);
  static bool parse_joint_config(const tinyxml2::XMLElement &element,
                                 JointInfo &info);

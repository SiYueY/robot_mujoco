...
struct RobotState {
  ...
  bool read_state(std::string name, CameraState& state) const { return find(cameras, name, state); }

 private:
  template <typename State>
  static bool find(const StateSnapshots<State>& states, const std::string& name, State& state) {
    if (states == nullptr) {
      return false;
    }

...
inline constexpr std::size_t MecanumWheelCount =
...
public:
  ...
  void forward(const Vector4d &wheel_angular, Vector3d &base_linear,
    ...
    const double front_left =
        wheel_angular[to_integer(MecanumWheelIndex::FrontLeft)];
    const double front_right =
        wheel_angular[to_integer(MecanumWheelIndex::FrontRight)];
    const double rear_left =
        wheel_angular[to_integer(MecanumWheelIndex::RearLeft)];
    const double rear_right =
        wheel_angular[to_integer(MecanumWheelIndex::RearRight)];

    constexpr double kQuarter = 0.25;
    const double forward = wheel_radius_ *
                           (front_left + front_right + rear_left + rear_right) *
                           kQuarter;
    const double lateral =
        wheel_radius_ * (-front_left + front_right + rear_left - rear_right) *
        kQuarter;
    const double yaw = wheel_radius_ *
                       (-front_left + front_right - rear_left + rear_right) /

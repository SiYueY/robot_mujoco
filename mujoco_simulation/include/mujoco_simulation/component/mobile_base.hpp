...
public:
  ...
  void inverse(const Vector3d &linear, const Vector3d &angular,
    ...
    wheel_angular = {(forward - lateral - yaw) / wheel_radius_,
                     (forward + lateral + yaw) / wheel_radius_,
                     (forward + lateral - yaw) / wheel_radius_,
                     (forward - lateral + yaw) / wheel_radius_};
  }
...
using MobileBaseInfo = MobileBaseConfig;
...
using MobileBaseCommandBatch = std::vector<std::optional<MobileBaseCommand>>;

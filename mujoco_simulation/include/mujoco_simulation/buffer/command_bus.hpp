...
#include "mujoco_simulation/buffer/command_channel.hpp"
...
// A CommandBus owns a collection of independently typed, fixed-layout command
// channels. Configuration is a cold-path operation: register every channel,
// then seal it with finalize_configuration(). New controllable component types
// only need their CommandTraits specialisation and one registered channel.
class MUJOCO_SIMULATION_PUBLIC CommandBus {
public:
...
private:
  ...
  template <typename Command> ChannelHolder<Command> *find_channel() {
    ...
    if (found == channels_.end())
      return nullptr;
    return static_cast<ChannelHolder<Command> *>(found->second.get());
  }

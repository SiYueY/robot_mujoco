...
本文档说明 `mujoco_simulation` 当前如何将 MuJoCo 的原生对象包装成统一的仿真设备接口。重点不是介绍通用 MuJoCo 用法，而是解释当前仓库中各类设备的工作原理、代码抽象方式，以及已经实现和暂未实现的边界。
...
### `ComponentManager` 的职责

`ComponentManager` 当前按设备分类维护组件注册表，并对上层提供统一调度能力。当前主要管理：
...
`Camera` 目前不再通过独立 manager 挂接，而是由 `Simulation` 直接配合 `CameraRenderer`、`CameraBuffer` 和 `SensorScheduler` 维护。

对已纳入 `ComponentManager` 的设备，manager 负责：
...
它本身不实现物理仿真，也不拥有控制策略，只负责组织组件、调度采样和聚合状态。
...
`JointConfig` 中的 `CommandInterfaceType` 则表示当前上层希望用哪种命令接口控制该关节：
...
- 最终组装到 `LidarSample::laser_scan.ranges`
...
- 通过差速或全向底盘的语义轮名字段解析出 traction joints
- 通过 `Joint::write()` 下发轮速
- 通过 `Joint::read()` 回读轮速

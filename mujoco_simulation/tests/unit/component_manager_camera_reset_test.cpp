...
class RetainingCameraRenderService final : public mujoco_simulation::CameraRenderService {
public:
    ...
    mujoco_simulation::CameraRenderSubmitResult submit(
        const mujoco_simulation::CameraRenderBatchRequest& request,
        ...
        return replace_pending_batch_
                   ? mujoco_simulation::CameraRenderSubmitResult::ReplacedPendingBatch
                   : mujoco_simulation::CameraRenderSubmitResult::Accepted;
    }
    mujoco_simulation::CameraRenderWaitStatus wait(
        const mujoco_simulation::CameraRenderTicket& ticket, std::chrono::milliseconds) override {
        return query(ticket);
    }
    mujoco_simulation::CameraRenderWaitStatus query(
        const mujoco_simulation::CameraRenderTicket& ticket) const override {
...
private:
    ...
    std::map<std::pair<std::uint64_t, std::uint64_t>, mujoco_simulation::CameraRenderBatchResult>
        completed_;

...
template <>
struct mujoco_simulation::CommandTraits<AuxiliaryCommand> {
    static bool validate(const AuxiliaryCommand& command) { return std::isfinite(command.target); }
};
...
bool has_effort_command(
    const mujoco_simulation::CommandSnapshot& snapshot, std::size_t id, double effort) {
...
bool has_twist_command(
    const mujoco_simulation::CommandSnapshot& snapshot, std::size_t id, double forward) {
    const auto* commands = snapshot.channel<mujoco_simulation::MobileBaseCommand>();
    return commands != nullptr && id < commands->size() &&
           (*commands)[id].mode == mujoco_simulation::MobileBaseControlMode::Twist &&
...
int main() {
    ...
    if (!check(
            !buffer.write(CommandBatch<JointCommand>{}),
        ...
        !check(
            buffer.configure_channel<JointCommand>({true, true, true}) &&
                buffer.configure_channel<mujoco_simulation::MobileBaseCommand>({true, true}) &&
    ...
    CommandBatch<mujoco_simulation::MobileBaseCommand> undersized_base;
    undersized_base.slots.resize(1);
    undersized_base.slots[0] = twist;
    ...
    CommandBatch<JointCommand> undersized_joints;
    undersized_joints.slots.resize(2);
    undersized_joints.slots[1] = effort;
    ...
    valid.slots[1] = lighter;
    valid.slots[2] = effort;
    ...
    if (!check(buffer.write(CommandBatch<JointCommand>{}), "empty command batch was rejected") ||
        !check(
    ...
    if (!check(
        ...
        !check(
            cleared.channel<JointCommand>()->at(2).mode ==
                    mujoco_simulation::JointControlMode::Effort &&
                cleared.channel<JointCommand>()->at(2).effort == 0.0,
    ...
    buffer.shutdown();
    if (!check(
            buffer.read_if_updated(sequence_before_shutdown, cleared),
            "shutdown did not update the command sequence") ||
        !check(
            cleared.channel<JointCommand>() == nullptr, "shutdown did not release command slots") ||
        !check(!buffer.write(2, effort), "shutdown buffer accepted a joint command")) {
        return 1;
    }

    if (!check(
            buffer.configure_channel<JointCommand>({true}) &&
                buffer.configure_channel<mujoco_simulation::MobileBaseCommand>({true}) &&
                buffer.finalize_configuration(),
        ...
        while (!writer_done.load()) {
            if (buffer.read_if_updated(sequence, snapshot)) {
                ...
                if (commands == nullptr || mobile_commands == nullptr ||
                    ...
                    !has_twist_command(snapshot, 0, mobile_commands->at(0).base_linear[0]) ||
    ...
    if (!check(
            buffer.configure_channel<AuxiliaryCommand>({true}) && buffer.finalize_configuration() &&
                buffer.write(0, auxiliary),
            "custom typed command channel could not be registered") ||
        !check(
            buffer.read().channel<AuxiliaryCommand>()->at(0).target == 3.0,

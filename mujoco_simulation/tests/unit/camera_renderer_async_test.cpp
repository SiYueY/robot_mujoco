...
int main() {
    mujoco_simulation_test::TemporaryFile model_file("mujoco_camera_async_test.xml");
    if (!check(
  ...
  <worldbody>
    ...
    bool released = false;
    {
        ...
        if (!check(
                renderer.query(*first_ticket, &first_result) ==
                        mujoco_simulation::CameraRenderWaitStatus::Completed &&
                    first_result.status == mujoco_simulation::CameraBatchStatus::Completed &&
        ...
        mujoco_simulation::CameraRenderBatchRequest moved_request = snapshot_request;
        ...
        if (!check(
                pending_ticket.has_value() && replacement_batch_ticket.has_value() &&
                    ...
                    pending_result.status == mujoco_simulation::CameraBatchStatus::Superseded &&
        ...
        waiter.join();
        if (!check(
                old_wait_result == mujoco_simulation::CameraRenderWaitStatus::InvalidTicket ||
                    old_wait_result == mujoco_simulation::CameraRenderWaitStatus::Stopped,
        ...
        if (!check(mixed_ticket.has_value(), "failed to submit mixed camera batch") ||
            ...
            !check(
                batch.cameras.size() == 2U && !batch.all_succeeded &&
                    batch.cameras[0].status == mujoco_simulation::CameraTaskStatus::Completed &&
                    batch.cameras[1].status == mujoco_simulation::CameraTaskStatus::Failed &&
    ...
    }
    {
        ...
        service_waiter.join();
        if (!check(
                service_wait_status == mujoco_simulation::CameraRenderWaitStatus::Stopped ||
                    service_wait_status == mujoco_simulation::CameraRenderWaitStatus::Cancelled,
        ...
        if (!check(
                reset_succeeded &&
                    (reset_wait_status == mujoco_simulation::CameraRenderWaitStatus::Stopped ||
                     reset_wait_status == mujoco_simulation::CameraRenderWaitStatus::Cancelled),

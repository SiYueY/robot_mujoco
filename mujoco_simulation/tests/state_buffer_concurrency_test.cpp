...
int main() {
  ...
  std::thread reader([&] {
    ...
    while (!finished.load()) {
      ...
      if (state != nullptr && state->sequence < previous)
        valid.store(false);
      if (state != nullptr)
  ...
  return check(valid.load(),
               "state sequence regressed under concurrent access") &&
                 check(final_state != nullptr &&
                           final_state->sequence == 10000U,

...
bool wait_for_camera_frame(mujoco_simulation::Simulation &simulation,
                           std::uint64_t minimum_sequence,
                           std::uint64_t *sequence = nullptr) {
  return mujoco_simulation_test::wait_until(
      [&] {
        std::shared_ptr<const mujoco_simulation::RobotState> state;
        if (!simulation.read_state(state) || state == nullptr ||
            state->cameras == nullptr || state->cameras->empty() ||
            (*state->cameras)[0] == nullptr) {
          return false;
        }
...
int main() {
  ...
  if (!check(simulation.read_state(state) && state != nullptr,
      ...
      !check(simulation.step_count() == 1,
             "public manual simulation step was not counted") ||

...
// Rotates a vector by the inverse of `orientation` ({w, x, y, z} order).
Vector3d inverse_rotate(const Quaterniond& orientation, const Vector3d& value) {
    const double inverse_w = orientation[0];
    const double inverse_x = -orientation[1];
    const double inverse_y = -orientation[2];
    const double inverse_z = -orientation[3];
    const Vector3d cross{
        2.0 * (inverse_y * value[2] - inverse_z * value[1]),
        2.0 * (inverse_z * value[0] - inverse_x * value[2]),
        2.0 * (inverse_x * value[1] - inverse_y * value[0])};
    return {
        value[0] + inverse_w * cross[0] + inverse_y * cross[2] - inverse_z * cross[1],
        value[1] + inverse_w * cross[1] + inverse_z * cross[0] - inverse_x * cross[2],
        value[2] + inverse_w * cross[2] + inverse_x * cross[1] - inverse_y * cross[0]};
}

std::vector<SwerveModuleGeometry> make_module_geometry(const SwerveMobileBaseInfo& info) {
    std::vector<SwerveModuleGeometry> geometry;
    geometry.reserve(info.modules.size());
...
: MobileBaseComponent(info.common.name, info.common.period),
  info_(std::move(info)),
  kinematics_(make_module_geometry(info_)) {}

bool DynamicSwerveMobileBase::init(const SimulationContext& context) {
    ...
    int free_joint = -1;
    for (int joint = 0; body_id >= 0 && joint < context.model->njnt; ++joint) {
    ...
    for (const SwerveModuleInfo& module_info : info_.modules) {
        Module module;
        module.steering_joint =
            mj_name2id(context.model, mjOBJ_JOINT, module_info.steering_joint_name.c_str());
        module.drive_joint =
            mj_name2id(context.model, mjOBJ_JOINT, module_info.drive_joint_name.c_str());
        module.steering_actuator =
            mj_name2id(context.model, mjOBJ_ACTUATOR, module_info.steering_actuator_name.c_str());
        module.drive_actuator =
            mj_name2id(context.model, mjOBJ_ACTUATOR, module_info.drive_actuator_name.c_str());
...
bool DynamicSwerveMobileBase::reset(const SimulationContext& context) {
    return ready_ && update(context);
}

bool DynamicSwerveMobileBase::write(
    const SimulationContext& context, const MobileBaseCommand& command) {
    ...
    if (!kinematics_.inverse(command.velocity, feedback_, targets_)) return false;
    for (std::size_t index = 0; index < modules_.size(); ++index) {
...
bool DynamicSwerveMobileBase::update(const SimulationContext& context) {
    if (!ready_) return false;
    const mjtNum* base_qpos = context.data->qpos + base_qpos_;
    const mjtNum* base_qvel = context.data->qvel + base_dof_;
    working_.id = info_.common.id;
    working_.timestamp = context.data->time;
    working_.pose.position = {base_qpos[0], base_qpos[1], base_qpos[2]};
    working_.pose.orientation = {base_qpos[3], base_qpos[4], base_qpos[5], base_qpos[6]};
    working_.twist.linear =
        inverse_rotate(working_.pose.orientation, {base_qvel[0], base_qvel[1], base_qvel[2]});
    // MuJoCo stores free-joint angular velocity in the joint's local frame.
    working_.twist.angular = {base_qvel[3], base_qvel[4], base_qvel[5]};

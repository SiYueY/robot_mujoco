# `romujoco` 现代 C++ / CMake 工程完整重构方案

## 1. 文档目的

本文档用于指导 `romujoco` 模块完成目录结构、公共 API、内部实现、Camera 批量渲染、符号可见性、CMake targets、安装包、静态链接和测试体系的系统性重构。

本次重构的总体目标是：

> 工程内部保持 Runtime、Render、Viewer 分层和清晰依赖；对外只提供一个完整、固定能力、易于使用的 `romujoco::romujoco` 库。
...
```cmake
find_package(romujoco REQUIRED)
...
```text
ROMUJOCO_BUILD_RENDER
ROMUJOCO_BUILD_VIEWER
```
...
```text
include/romujoco/config_result.hpp
include/romujoco/simulation_result.hpp
```
...
实际头文件以 `ROMUJOCO_PUBLIC` 标注每个非内联构造、析构和成员
函数，而不是标注整个类；该宏由最终 `romujoco` target 的
...
```text
romujoco_runtime_objects
romujoco_render_objects
romujoco_viewer_objects
```
...
```cmake
option(
  ROMUJOCO_BUILD_SHARED
  "Build shared library"
  ON
)

option(
  ROMUJOCO_BUILD_TESTS
  "Build tests"
  ${ROMUJOCO_IS_TOP_LEVEL}
)

option(
  ROMUJOCO_BUILD_EXAMPLES
  "Build examples"
  ${ROMUJOCO_IS_TOP_LEVEL}
)
...
option(
  ROMUJOCO_ENABLE_ASAN
  "Enable AddressSanitizer"
  OFF
)

option(
  ROMUJOCO_ENABLE_UBSAN
...
include(
  "${CMAKE_CURRENT_LIST_DIR}/romujocoTargets.cmake"
)
```
...
install(
  ...
  DESTINATION
    "${CMAKE_INSTALL_INCLUDEDIR}/romujoco"
)
```
...
```text
romujoco_runtime          OBJECT
romujoco_render           OBJECT
romujoco_viewer           OBJECT
```

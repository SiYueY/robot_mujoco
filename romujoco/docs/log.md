...
本文定义 `romujoco` 日志模块 V1 的冻结设计，包括公共 API、日志宏、compile-time filtering、runtime admission、lazy stream、默认 `simulate.log`、backend 状态机、并发同步、运行期 sink failure、ABI、文本安全、Easylogging++ 隔离、CMake 和测试架构。
...
```cpp
namespace romujoco::logging::impl {
}
```
...
```cpp
romujoco::logging::set_level(
    romujoco::logging::Level::Off);
```
...
```text
stderr 永远不输出 romujoco 生成的 ANSI color sequence
```
...
#define SIMULATE_LOG(level)                                          \
    !::romujoco::logging::is_enabled(level) ? (void)0 :    \
...
因此同一 process 内不会有两个 romujoco backend operations 并行执行。
...
> 同一 process 内，由 `romujoco` 发出的两条日志不会在同一 sink 上发生字节级交错。
...
它仅负责 Easylogging++ 自身 storage，不代表 `romujoco` runtime bootstrap。
...
必须在独立 consumer 中只依赖该 header 及 `romujoco::romujoco` 正常编译。

...
int main(int argc, char** argv) {
    ...
    if (!romujoco::logging::configure(policy)) return 9;
    SIM_INFO << "first-cwd";
    ...
    if (first_contents.find("first-cwd") == std::string::npos ||
        ...
        return 11;
    if (!romujoco::logging::configure(policy)) return 12;
    SIM_INFO << "second-cwd";
    romujoco::logging::flush();

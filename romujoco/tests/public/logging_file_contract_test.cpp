...
int main() {
    ...
    if (!romujoco::logging::configure(policy)) return 2;

    policy.file_enabled = true;
    policy.file_path.clear();
    if (romujoco::logging::configure(policy)) return 3;

    policy.file_path = (temporary.path / "missing" / "file.log").string();
    if (romujoco::logging::configure(policy)) return 4;

    policy.file_path = temporary.path.string();
    if (romujoco::logging::configure(policy)) return 5;
    ...
    if (romujoco::logging::configure(policy)) return 8;
    ...
    const bool accepted_socket = socket_available && romujoco::logging::configure(policy);
    ::close(socket_fd);
    ...
    if (!romujoco::logging::configure(policy)) return 13;
    ...
    if (!romujoco::logging::configure(policy)) return 16;
    ...
    return romujoco::logging::configure(policy) ? 18 : 0;
}

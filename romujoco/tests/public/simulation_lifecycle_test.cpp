...
bool wait_for_camera_frame(
    romujoco::Simulation& simulation, std::uint64_t minimum_sequence,
    std::uint64_t* sequence = nullptr) {
    return romujoco_test::wait_until(
        [&] {
            std::shared_ptr<const romujoco::RobotState> state;
...
int main() {
    romujoco_test::TemporaryFile model("romujoco_lifecycle_test.xml");
    if (!check(
  ...
  <keyframe><key name="home"/></keyframe>
    ...
    if (!check(simulation.initialize(config), "simulation initialization failed") ||
        ...
        !check(
            simulation.write_command(romujoco::RobotCommand{}),
            "public empty robot command was rejected")) {
        return 1;
    }

    std::shared_ptr<const romujoco::RobotState> state;
    romujoco::CameraState camera_state;
    ...
    if (!check(
        ...
        !check(
            simulation.status() == romujoco::SimulationStatus::Paused,
        ...
        !check(
            simulation.status() == romujoco::SimulationStatus::Error,
        ...
        !check(
            simulation.status() == romujoco::SimulationStatus::Uninitialized,

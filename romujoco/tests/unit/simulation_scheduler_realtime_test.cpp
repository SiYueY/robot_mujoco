...
int main() {
    ...
    romujoco::SimulationScheduler scheduler;
    ...
    romujoco::SimulationScheduler no_catchup_scheduler;
    ...
    romujoco::SimulationScheduler pause_scheduler;
    ...
    if (!check(pause_result, "synchronous pause failed") ||
        !check(
            pause_scheduler.status() == romujoco::SimulationStatus::Paused,
    ...
    romujoco::SimulationScheduler self_stop_scheduler;
    ...
    while (self_stop_scheduler.status() != romujoco::SimulationStatus::Stopped &&
           std::chrono::steady_clock::now() < stop_deadline) {
        std::this_thread::sleep_for(1ms);
    }
    if (!check(self_stop_rejected.load(), "scheduler worker joined itself") ||
        !check(
            self_stop_scheduler.status() == romujoco::SimulationStatus::Stopped,
    ...
    romujoco::SimulationScheduler normal_stop_scheduler;
    ...
    if (!check(
        ...
        !check(
            normal_stop_scheduler.status() != romujoco::SimulationStatus::Error,
    ...
    romujoco::SimulationScheduler failing_scheduler;
    if (!check(
    ...
    while (failing_scheduler.status() != romujoco::SimulationStatus::Error &&
           std::chrono::steady_clock::now() < error_deadline) {
        std::this_thread::sleep_for(1ms);
    }
    if (!check(
            failing_scheduler.status() == romujoco::SimulationStatus::Error,
    ...
    romujoco::SimulationScheduler throwing_scheduler;
    if (!check(
    ...
    while (throwing_scheduler.status() != romujoco::SimulationStatus::Error &&
           std::chrono::steady_clock::now() < error_deadline) {
        std::this_thread::sleep_for(1ms);
    }
    if (!check(
            throwing_scheduler.status() == romujoco::SimulationStatus::Error,
    ...
    romujoco::SimulationScheduler stop_scheduler;
    ...
    if (!check(stop_result, "stop scheduler stop failed") ||
        !check(
            stop_scheduler.status() == romujoco::SimulationStatus::Stopped,

...
int main() {
    const std::filesystem::path path =
        std::filesystem::temp_directory_path() / "romujoco_timing_config_test.xml";
...
<joint id="0" name="joint" mode="position"><control>
  ...
  <effort gravity_compensation="true"/>
    ...
    const auto& gravity_joint = std::get<romujoco::JointInfo>(config.components[0]);
    if (!check(
...
<joint id="0" name="joint" mode="effort"><control><effort/></control><limit><position min="-2.8"/><velocity max="5"/></limit></joint>
    ...
    const auto& one_sided_joint = std::get<romujoco::JointInfo>(config.components[0]);
    if (!check(
...
<robot>
  ...
  <joint id="0" name="second"/>
    ...
    if (!check(
            !romujoco::SimulationConfigValidator::validate(invalid_period_config),
            "validator accepted a negative direct C++ component period")) {
        cleanup();
        return 1;
    }

    romujoco::SimulationConfig duplicate_config;
    duplicate_config.model.model_path = "model.xml";
    invalid_period_joint.period = 0.001;
    duplicate_config.components.emplace_back(invalid_period_joint);
    duplicate_config.components.emplace_back(invalid_period_joint);
    if (!check(
            !romujoco::SimulationConfigValidator::validate(duplicate_config),
            "validator accepted duplicate direct C++ component IDs")) {
        cleanup();
        return 1;
    }

    romujoco::SimulationConfig missing_model_config;
    if (!check(
            !romujoco::SimulationConfigValidator::validate(missing_model_config),
            "validator accepted an empty direct C++ model path")) {
        cleanup();
        return 1;
    }

    romujoco::SimulationConfig missing_camera_name_config;
    missing_camera_name_config.model.model_path = "model.xml";
    romujoco::CameraConfig missing_camera_name;
    ...
    if (!check(
            !romujoco::SimulationConfigValidator::validate(missing_camera_name_config),
            "validator omitted a camera name diagnostic")) {
        cleanup();
        return 1;
    }

    romujoco::SimulationConfig invalid_renderer_config;
    invalid_renderer_config.model.model_path = "model.xml";
    invalid_renderer_config.camera_renderer.completed_ticket_history = 0U;
    if (!check(
            !romujoco::SimulationConfigValidator::validate(invalid_renderer_config),
            "validator accepted zero camera ticket history")) {
        cleanup();
        return 1;
    }

    romujoco::SimulationConfig no_backend_config;
    ...
    if (!check(
            !romujoco::SimulationConfigValidator::validate(no_backend_config),
            "validator accepted a camera renderer without a backend")) {
        cleanup();
        return 1;
    }

    romujoco::SimulationConfig missing_lidar_name_config;
    ...
    if (!check(
            !romujoco::SimulationConfigValidator::validate(missing_lidar_name_config),
    ...
    if (!check(
            !romujoco::SimulationConfigValidator::validate(missing_base_name_config),

...
int main() {
    romujoco_test::TemporaryFile model_file("mujoco_camera_snapshot_tsan_test.xml");
    if (!check(
            model_file.write(R"(<mujoco model="camera_snapshot_tsan_test">
  <worldbody>
    <light pos="0 0 3"/>
    ...
    const bool success = !submit_failed.load(std::memory_order_acquire) &&
                         renderer.snapshot_copy_count() == 48U && renderer.release();
    context.clear();
    return success ? 0 : 1;
}

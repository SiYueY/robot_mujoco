...
int main() {
    romujoco_test::TemporaryFile model_file("mujoco_camera_async_test.xml");
    if (!check(
            model_file.write(R"(<mujoco model="camera_async_test">
  <worldbody>
    <light pos="0 0 3"/>
    ...
    bool released = false;
    {
        ...
        if (!check(renderer.initialize(context), "failed to initialize camera worker")) {
            return 1;
        }
        ...
        romujoco::CameraRenderTask task;
        task.config.id = 2;
        ...
        romujoco::CameraRenderBatchRequest first_request;
        ...
        romujoco::CameraBatchResult first_result;
        if (!check(
                renderer.query(*first_ticket, &first_result) ==
                        romujoco::CameraRenderWaitStatus::Completed &&
                    first_result.status == romujoco::CameraBatchStatus::Completed &&
        ...
        romujoco::CameraRenderTask blocker = task;
        blocker.config.width = 1024;
        blocker.config.height = 768;
        blocker.sequence = 20;
        if (!check(
                renderer.submit(context, {blocker}).has_value(),
                "failed to submit snapshot-isolation blocker")) {
            renderer.release();
            return 1;
        }
        romujoco::CameraRenderTask snapshot_task = task;
        snapshot_task.sequence = 21;
        romujoco::CameraRenderBatchRequest snapshot_request;
        ...
        if (!check(
                snapshot_ticket.has_value() && model->nq > 0,
        ...
        data->qpos[0] = 2.0;
        mj_forward(model, data);
        romujoco::CameraBatchResult snapshot_result;
        if (!check(
                renderer.wait(*snapshot_ticket, &snapshot_result) &&
                    snapshot_result.cameras.size() == 1U,
                "snapshot-isolation request did not finish")) {
            renderer.release();
            return 1;
        }
        romujoco::CameraRenderBatchRequest moved_request = snapshot_request;
        ...
        romujoco::CameraBatchResult moved_result;
        if (!check(
                moved_ticket.has_value() && renderer.wait(*moved_ticket, &moved_result) &&
        ...
        romujoco::CameraRenderTask active_task = task;
        ...
        const auto active_deadline = std::chrono::steady_clock::now() + std::chrono::seconds(5);
        while (renderer.active_ticket_sequence_for_test() != active_ticket->sequence &&
               std::chrono::steady_clock::now() < active_deadline) {
            std::this_thread::yield();
        }
        if (!check(
                renderer.active_ticket_sequence_for_test() == active_ticket->sequence,
                "camera worker did not begin the active batch")) {
            renderer.release();
            return 1;
        }
        romujoco::CameraRenderTask pending_task = task;
        pending_task.sequence = 31;
        const auto pending_ticket = renderer.submit(context, {pending_task});
        romujoco::CameraRenderTask replacement_task = task;
        ...
        const auto replacement_batch_ticket = renderer.submit(
            romujoco::CameraRenderBatchRequest{
                0, 0, 0, data->time, model, data, {replacement_task}},
            &replacement_replaced_pending_batch);
        romujoco::CameraBatchResult pending_result;
        if (!check(
                pending_ticket.has_value() && replacement_batch_ticket.has_value() &&
                    replacement_replaced_pending_batch &&
                    renderer.wait_result(*pending_ticket, &pending_result) ==
                        romujoco::CameraRenderWaitStatus::Superseded &&
                    pending_result.status == romujoco::CameraBatchStatus::Superseded &&
                    pending_result.cameras.size() == 1U &&
                    pending_result.cameras.front().status ==
                        romujoco::CameraTaskStatus::Superseded &&
                    renderer.wait(*active_ticket) && renderer.wait(*replacement_batch_ticket),
                "batch supersede did not preserve active and replace pending")) {
            renderer.release();
            return 1;
        }

        romujoco::CameraRenderTask second_camera = task;
        ...
        const auto two_camera_ticket = renderer.submit(context, {task, second_camera});
        romujoco::CameraBatchResult two_camera_result;
        if (!check(
                two_camera_ticket.has_value() && renderer.wait(*two_camera_ticket) &&
                    ...
                    std::all_of(
                        two_camera_result.cameras.begin(), two_camera_result.cameras.end(),
                        [&two_camera_result](const auto& camera) {
                            return camera.generation == two_camera_result.ticket.generation &&
                                   camera.batch_sequence == two_camera_result.ticket.sequence &&
                                   camera.simulation_step == two_camera_result.simulation_step &&
                                   camera.simulation_time == two_camera_result.simulation_time;
                        }),
        ...
        if (!check(
                restarted_ticket.has_value() && restarted_ticket->sequence == old_ticket.sequence &&
        ...
        if (!check(renderer.release(), "failed to prepare race-test lifecycle") ||
            !check(renderer.initialize(context), "failed to initialize race-test lifecycle")) {
            return 1;
        }
        romujoco::CameraRenderTask race_task = task;
        ...
        romujoco::CameraRenderWaitStatus old_wait_result =
        ...
        if (!check(renderer.release(), "failed to release race-test lifecycle") ||
            !check(renderer.initialize(context), "failed to restart race-test lifecycle")) {
            waiter.join();
            return 1;
        }
        ...
        if (!check(
            ...
            !check(
                renderer.wait(*replacement_ticket), "race-test replacement frame did not finish")) {
            waiter.join();
            renderer.release();
            return 1;
        }
        waiter.join();
        if (!check(
                old_wait_result == romujoco::CameraRenderWaitStatus::InvalidTicket ||
                    old_wait_result == romujoco::CameraRenderWaitStatus::Stopped,
        ...
        if (!check(intermediate_ticket.has_value(), "failed to submit intermediate frame")) {
            renderer.release();
            return 1;
        }
        task.sequence = 5;
        ...
        romujoco::CameraBatchResult latest_result;
        if (!check(
                renderer.query(*latest_ticket, &latest_result) ==
                        romujoco::CameraRenderWaitStatus::Completed &&
        ...
        romujoco::CameraBatchResult intermediate_result;
        const bool intermediate_succeeded =
            renderer.wait(*intermediate_ticket, &intermediate_result);
        if (!check(
                intermediate_result.ticket.generation == intermediate_ticket->generation &&
                    intermediate_result.ticket.sequence == intermediate_ticket->sequence,
                "intermediate ticket returned a different batch result") ||
            !check(
                intermediate_succeeded || (!intermediate_result.all_succeeded &&
        ...
        romujoco::CameraRenderTask out_of_range = task;
        out_of_range.config.id = 256;
        out_of_range.camera_id = 256;
        romujoco::CameraRenderTask duplicate = task;
        duplicate.sequence = 4;
        if (!check(
                !renderer.submit(context, {out_of_range}), "out-of-range camera ID was accepted") ||
            !check(
                !renderer.submit(context, {task, duplicate}), "duplicate camera ID was accepted")) {
            renderer.release();
            return 1;
        }

        romujoco::CameraRenderTask failed = task;
        failed.config.id = 3;
        failed.camera_id = 3;
        failed.config.camera_name = "missing_camera";
        failed.sequence = 5;
        task.sequence = 6;
        romujoco::CameraBatchResult batch;
        const auto mixed_ticket = renderer.submit(context, {task, failed});
        if (!check(mixed_ticket.has_value(), "failed to submit mixed camera batch") ||
            !check(
                renderer.wait_result(*mixed_ticket, &batch) ==
                    romujoco::CameraRenderWaitStatus::PartiallyFailed,
                "mixed camera batch did not report PartiallyFailed") ||
            !check(
                renderer.query(*mixed_ticket, &batch) ==
                    romujoco::CameraRenderWaitStatus::PartiallyFailed,
                "mixed camera batch query did not report PartiallyFailed") ||
            !check(
                !renderer.wait(*mixed_ticket, &batch),
                "mixed camera batch was incorrectly successful") ||
            !check(
                batch.cameras.size() == 2U && !batch.all_succeeded &&
                    batch.cameras[0].status == romujoco::CameraTaskStatus::Completed &&
                    batch.cameras[1].status == romujoco::CameraTaskStatus::Failed &&
        ...
        std::thread camera_submitter([&] {
            for (std::uint64_t index = 0; index < 24; ++index) {
                romujoco::CameraRenderTask concurrent_task = task;
                concurrent_task.sequence = 1000 + index;
                romujoco::CameraRenderBatchRequest request;
        ...
        if (!check(
                !concurrent_submit_failed.load(std::memory_order_acquire),
    ...
    }
    {
        ...
        romujoco::CameraRendererConfig service_renderer_config;
        service_renderer_config.allow_glfw_backend = false;
        service_renderer_config.allow_egl_backend = true;
        romujoco::CameraRenderServiceImpl service(service_renderer_config);
        romujoco::SimulationConfig service_config;
        if (!check(
                service.initialize(service_config, model),
                "failed to initialize camera render service")) {
            context.clear();
            return 1;
        }
        romujoco::CameraRenderTask service_task;
        ...
        romujoco::CameraRenderBatchRequest service_request;
        ...
        romujoco::CameraRenderTicket service_ticket;
        if (!check(
        ...
        romujoco::CameraRenderWaitStatus service_wait_status =
            romujoco::CameraRenderWaitStatus::Timeout;
        std::thread service_waiter([&] {
            service_waiter_started.store(true, std::memory_order_release);
            service_wait_status = service.wait(service_ticket, std::chrono::seconds(5));
        });
        ...
        service_waiter.join();
        if (!check(
                service_wait_status == romujoco::CameraRenderWaitStatus::Stopped ||
                    service_wait_status == romujoco::CameraRenderWaitStatus::Cancelled ||
                    service_wait_status == romujoco::CameraRenderWaitStatus::Completed ||
                    service_wait_status == romujoco::CameraRenderWaitStatus::InvalidTicket,
                "service shutdown neither completed nor woke the batch waiter")) {
            context.clear();
            return 1;
        }
        if (!check(
        ...
        service_request.tasks = {service_task};
        const bool reset_submit_accepted = service.submit(service_request, service_ticket) ==
                                           romujoco::CameraRenderSubmitResult::Accepted;
        const bool reset_succeeded = reset_submit_accepted && service.reset();
        const auto reset_wait_status =
            reset_succeeded ? service.wait(service_ticket, std::chrono::milliseconds(1))
                            : romujoco::CameraRenderWaitStatus::Failed;
        if (!check(
                reset_succeeded &&
                    (reset_wait_status == romujoco::CameraRenderWaitStatus::Stopped ||
                     reset_wait_status == romujoco::CameraRenderWaitStatus::Cancelled),
                "service reset did not invalidate the pre-reset batch")) {
            service.shutdown();
            context.clear();
            return 1;
        }
        if (!check(service.shutdown(), "camera render service final shutdown failed")) {
            context.clear();
            return 1;
        }
    }
    romujoco::CameraRendererConfig concurrent_config;
    concurrent_config.allow_glfw_backend = false;
    concurrent_config.allow_egl_backend = true;
    romujoco::CameraRenderer concurrent_renderer(concurrent_config);
    ...
    if (!check(
            first_initialized.load() && second_initialized.load(),
    ...
    std::thread first_releaser([&] { first_released.store(concurrent_renderer.release()); });
    std::thread second_releaser([&] { second_released.store(concurrent_renderer.release()); });
    first_releaser.join();
    second_releaser.join();
    if (!check(
            first_released.load() && second_released.load() &&
    ...
    romujoco::CameraRendererConfig history_config;
    history_config.allow_glfw_backend = false;
    history_config.allow_egl_backend = true;
    history_config.completed_ticket_history = 1U;
    romujoco::CameraRenderer history_renderer(history_config);
    if (!check(
            history_renderer.initialize(context), "failed to initialize ticket-history renderer")) {
        context.clear();
        return 1;
    }
    romujoco::CameraRenderTask history_task;
    history_task.config.id = 2;
    history_task.camera_id = 2;
    ...
    if (!check(
            stale_ticket.has_value() && history_renderer.wait(*stale_ticket),
    ...
    if (!check(history_renderer.release(), "failed to release ticket-history renderer")) {
        context.clear();
        return 1;
    }
    context.clear();

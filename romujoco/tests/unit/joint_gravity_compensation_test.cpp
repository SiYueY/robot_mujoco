...
romujoco::JointInfo joint_info(bool gravity_compensation) {
...
int main() {
    romujoco_test::TemporaryFile model_file("mujoco_joint_gravity_compensation_test.xml");
    if (!check(
...
<worldbody><body name="pendulum"><joint name="joint" type="hinge" axis="0 1 0"/>
  <inertial pos="0 0 -0.5" mass="1" diaginertia="0.1 0.1 0.1"/>
    ...
    romujoco::JointComponent component(joint_info(true));
    if (!check(component.init(context), "failed to initialize gravity-compensated joint")) {
        context.clear();
        return 1;
    }

    romujoco::JointCommand command;
    command.id = 0;
    command.mode = static_cast<std::uint8_t>(romujoco::JointMode::Position);
    ...
    command.mode = static_cast<std::uint8_t>(romujoco::JointMode::Velocity);
    ...
    command.mode = static_cast<std::uint8_t>(romujoco::JointMode::Hybrid);
    command.position = qpos_before;
    command.velocity = qvel_before;
    command.effort = 0.0;
    ...
    command.mode = static_cast<std::uint8_t>(romujoco::JointMode::Effort);
    command.effort = 0.0;
    ...
    mj_forward(model, data);
    romujoco::JointComponent disabled_component(joint_info(false));
    ...
    romujoco::JointComponent limited_component(std::move(limited_info));
    ...
    romujoco::JointComponent continuous_component(std::move(continuous_info));
    data->qpos[0] = 3.13;
    data->qvel[0] = 0.0;
    mj_forward(model, data);
    command.mode = static_cast<std::uint8_t>(romujoco::JointMode::Position);
    ...
    romujoco::JointInfo invalid_passive;
    invalid_passive.id = 1;
    invalid_passive.joint_name = "joint";
    invalid_passive.actuation = romujoco::JointActuation::Passive;
    invalid_passive.default_mode = romujoco::JointMode::None;
    invalid_passive.period = 0.001;
    romujoco::JointComponent passive_with_actuator(std::move(invalid_passive));
    if (!check(
